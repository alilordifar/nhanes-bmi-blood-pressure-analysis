...
Finally, we extend the interaction model by adding a quadratic age term (Age²) to test whether blood pressure rises with age at a constant rate or accelerates at older ages. This specification keeps the BMI × Age interaction from Model 4 while allowing the age-blood pressure relationship to curve rather than follow a straight line.
...
model_five   <- lm(bp ~ bmi * age + smoking + I(age^2), data = df)
```
...
stargazer(
  ...
  covariate.labels = c("BMI", "BMI²", "Age", "Smoking", "Age²", "BMI × Age"),
  digits = 2,
  omit.stat = c("ser", "f"),
...
For Model 4, which includes an interaction between BMI and age, the residuals appear very similar to Model 2. There is no clear improvement or deterioration in the residual patterns, meaning that while the interaction adds flexibility to the model, it does not significantly change the underlying distribution of residuals.

For Model 5, adding the quadratic age term does not noticeably change the residual pattern compared to Models 2 and 4. The residual spread remains similar, and the same mild heteroscedasticity and tail deviations persist, consistent with the other specifications.
...
In Model 4, including an interaction term between BMI and age shows that the effect of BMI on blood pressure depends on age. The interaction coefficient is negative and statistically significant, indicating that BMI has a stronger association with blood pressure at younger ages and a weaker association at older ages. While the overall model fit remains similar to Model 2, this specification provides insight on how BMI and age jointly relates to blood pressure.

In Model 5, adding a quadratic age term to the interaction model reveals that the age-blood pressure relationship accelerates at older ages. The Age² coefficient is positive and statistically significant (p < 0.001), indicating that blood pressure does not just increase steadily with age but steepens over time. The BMI × Age interaction remains significant and negative, confirming that BMI has a smaller association with blood pressure among older adults. Smoking remains nonsignificant. The adjusted R² increases slightly from 0.2622 in Model 4 to 0.2644, suggesting a modest improvement in explanatory power.

## Overall Effect

This analysis finds a consistent, positive association between BMI and systolic blood pressure — higher BMI tends to go with higher blood pressure, and this holds even after controlling for age and smoking. However, the effect is modest: BMI alone explains only 1.8% of the variation in blood pressure, while age turns out to be a much stronger predictor. The polynomial model also showed no meaningful curvature, suggesting the relationship is essentially linear. Incorporating an interaction between BMI and age further shows that the strength of the relationship varies across individuals, with BMI having a stronger association with blood pressure at younger ages and a weaker association at older ages. Adding a quadratic age term confirms that the age-blood pressure relationship is not constant — it steepens at older ages, meaning blood pressure rises more rapidly later in life. These findings give a clear descriptive picture of how BMI and blood pressure move together in the U.S. adult population.
...
p5 <- make_resid_plot(model_five, "Model 5")
